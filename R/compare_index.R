...
compare_index <- function(data = NULL,
  ...
  index$og_indx_st %>%
    ...
    tidytable::drop_na() %>%
    tidytable::mutate(diff_biom = tidytable::case_when(biomass_mt_gap > 0 ~ (biomass_mt_gap - biomass_mt) / biomass_mt_gap,
                                            .default = 0),
                      diff_biom_var = tidytable::case_when(biomass_var_gap > 0 ~ (biomass_var_gap - biomass_var) / biomass_var_gap,
                                                .default = 0),
                      diff_popn = tidytable::case_when(population_count_gap > 0 ~ (population_count_gap - population_count) / population_count_gap,
                                            .default = 0),
                      diff_popn_var = tidytable::case_when(population_var_gap > 0 ~ (population_var_gap - population_var) / population_var_gap,
                                                .default = 0)) %>%
