#' Re-stratify haul locations
#'
#' @description
#' Function that places historical hauls into the new GOA survey strata. Prior to running this function you will need to
#' run `query_data()` to obtain the list of necessary data (i.e., hauls) as an input to re-stratify the historical hauls.
...
#' @param run_terra boolean, if TRUE then runs the terra::extract() function to re-stratify hauls,
...
restrat <- function(data = NULL,
  ...
  if(isTRUE(run_terra)){
    ...
    file.copy(system.file("goa_strata_2025.gpkg", package = "goaRestrat"),
              here::here("data"))
    ...
    new_haul <- vroom::vroom(new_haul, here::here('data', 'new_haul.csv'),
                                  progress = FALSE,
